...
//         <button onClick={() => { handlePage(-1) }} disabled={page==1} > <b>&lt;</b> </button> &nbsp;&nbsp;&nbsp;&nbsp;
//         <button disabled >{page}</button>&nbsp;&nbsp;&nbsp;&nbsp;
//         <button onClick={() => { handlePage(1) }} > <b>&gt;</b> </button>
//       </div>
//     </div>
//   );
// }
...
function Movies() {
  ...
  useEffect(() => {
    if (searchTerm === "") {
      // Fetch now-playing movies if there's no search term
      dispatch(requestAction());
      dispatch(
        requestMovieDataFetch(
      ...
      dispatch(
        requestMovieDataFetch(
          `https://api.themoviedb.org/3/search/movie?api_key=5ae304b91cd12d71e100db44c6812cb6&language=en-US&query=${searchTerm}&page=${page}`
        )
      );
    }
  ...
  return (
    <div className={styles.box}>
      <div className={styles.navbar}>
        <Navbar text="movies" onSearchChange={handleSearchChange} />
      </div>
      ...
      <div className={styles.container}>
        {loading ? (
          ...
          data &&
          data.map((el, ind) => <Component key={el.id} {...el} IDe={ind} />)
        )}
      </div>
      <div className={styles.pagination}>
